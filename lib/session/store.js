...
Store.prototype.set = function (sessionId, session, callback) {
  ...
  recordGet(this.coll, sessionId, { thrownNotFound: false })
    .then(item => {
      if (!item) return recordCreate(this.coll, { id: sessionId, session: sess }, { skipHook: true, skipValidation: true })
      return recordUpdate(this.coll, sessionId, { session: sess }, { skipHook: true, skipValidation: true })
    })
    .then(item => {
      callback()
    })
...
Store.prototype.get = function (sessionId, callback) {
  ...
  recordGet(this.coll, sessionId, { skipHook: true, skipValidation: true })
    .then(item => {
...
Store.prototype.destroy = function (sessionId, callback) {
  ...
  recordRemove(this.coll, sessionId, { skipHook: true, skipValidation: true })
    .then(item => {
      callback()
    })
    .catch(err => {
