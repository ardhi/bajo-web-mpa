...
class Component {
  constructor (plugin, $, theme) {
    ...
    this.$ = $
    this.theme = theme
    this.cacheMaxAge = plugin.app.bajoWebMpa.config.theme.component.cacheMaxAge
  }
  ...
  async t (params, reply) {
    ...
    const { attrToObject, attrToArray } = this.plugin.app.bajoWebMpa
  ...
  async include (params, reply) {
    /*
    const { getTplAndTheme } = this.plugin.app.bajoWebMpa
    ...
    */
    params.noTag = true
    const { render } = this.plugin.app.bajoWebMpa
  ...
  async script (params) {
    const { routePath } = this.plugin.app.bajoWebStatic
